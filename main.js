...
axios.defaults.headers.common['arfins-token']=
'this is my token';
 ...
 // GET REQUEST
function getTodos() {
  axios.get('https://jsonplaceholder.typicode.com/todos').then(res=>showOutput(res)).catch(err=> console.log(err));
}
...
function cancelToken() {
  ...
  .catch(thrown=>{
    if(axios.isCancel(thrown)){
      console.log('Reques canceled',thrown.message);
    }
  });
  if(true){
...
axios.interceptors.request.use(
  config=>{
    console.log(
      `${config.method.toUpperCase()} request send 
      to ${config.url} 
      at ${new Date().getTime()}`
    );
    return config;
